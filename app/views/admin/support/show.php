    ...
    <div class="col-lg-8">
        <div class="surface-card p-4 mb-3">
            <div class="d-flex justify-content-between align-items-start">
                ...
                <form method="post" action="<?= base_url('admin/support/' . $ticket['id'] . '/close') ?>" onsubmit="return confirm('Close this ticket?')">
        ...
        <div class="surface-card p-4 mb-3">
            <?php foreach ($messages as $m): ?>
                <div class="d-flex <?= $m['sender_type'] === 'admin' ? 'justify-content-end' : 'justify-content-start' ?> mb-3">
                    <div class="p-3 rounded-3" style="max-width:75%; background:<?= $m['sender_type'] === 'admin' ? 'var(--c-primary)' : 'var(--bg-surface-alt)' ?>; color:<?= $m['sender_type'] === 'admin' ? '#fff' : 'var(--text-primary)' ?>;">
                        <div class="small fw-semibold mb-1"><?= $m['sender_type'] === 'admin' ? 'Support Team' : 'User' ?></div>
        ...
        <div class="surface-card p-4">
            <form method="post" action="<?= base_url('admin/support/' . $ticket['id'] . '/reply') ?>" enctype="multipart/form-data">
                <?= csrf_field() ?>
