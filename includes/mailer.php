...
HTML;
    ...
    public static function sendVerificationEmail(string $email, string $name, string $link): bool
    {
        $body = "
            ...
            <p style='text-align:center;margin:28px 0;'>
              <a href='{$link}' style='background:#0F5132;color:#ffffff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block;'>Verify Email Address</a>
            </p>
    ...
    {
        $body = "
            <h2 style='margin-top:0;color:#0B2545;'>Welcome to 9JACASH, {$name}!</h2>
    ...
    public static function sendPasswordResetEmail(string $email, string $name, string $link): bool
    {
        $body = "
            ...
            <p style='text-align:center;margin:28px 0;'>
              <a href='{$link}' style='background:#0F5132;color:#ffffff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block;'>Reset Password</a>
            </p>
            <p>Or copy and paste this link into your browser:<br><a href='{$link}'>{$link}</a></p>
    ...
    {
        $body = "
            <h2 style='margin-top:0;color:#0B2545;'>Deposit {$status}</h2>
            <p>Hi {$name},</p>
            <p>Your deposit of <strong>" . money($amount) . "</strong> has been <strong>{$status}</strong>.</p>
        ";
    ...
    {
        $body = "
            <h2 style='margin-top:0;color:#0B2545;'>Withdrawal {$status}</h2>
            <p>Hi {$name},</p>
            <p>Your withdrawal request of <strong>" . money($amount) . "</strong> has been <strong>{$status}</strong>.</p>
        ";
